      ...
      success: function (resp) {
        if(resp['cambio']<0)
        {
          ...
          if(impuesto)
          {
            ...
            "<label class='text-success sin-margen'>Total: $"+miFormato(resp['total'])+"</label>"+
            "<br>"+
            "<label class='text-warning sin-margen'>Recibido: $"+miFormato(resp['recibido'])+"</label>"+
            "<br>"+
            "<label class='text-primary sin-margen'>Cambio: $"+miFormato(resp['cambio'])+"</label>"+
  ...
  $(document).on('click', '.delFila', function (event) {
      ...
      $.ajax({
        ...
        success: function (resultado) {
          ...
          $("#miTotal").html(   miFormato(nuevoTotal)   );
          if(impuesto=="1"){
        ...
        if(resultado)
        {
          ...
          $.ajax({
            ...
            success: function (resultadoItem) {
              ...
              // }
              $('#tbventa tr:last').after("<tr><td>"+resultadoItem['codigo']+"</td><td>"+resultadoItem['descripcion']+"</td><td>$ "+miFormato(precioImprimible)+"</td><td>"+cantidad+"</td><td>$ "+miFormato(importeImprimible)+"</td><td class='text-center'><button class='delFila btn btn-danger btn-xs'><i class='fa fa-times-circle' aria-hidden='true'></i></button></td><td class='hidden'>"+importe+"</td></tr>");
              $("#cantidad").val(1);
              $("#codigo").val("");
              ...
              $.ajax({
                ...
                success: function (r) {
                  ...
                  if(r['validante']=="r")
                  {
                    ...
                    $('#tbventa tr:last').after("<tr><td>"+resultadoItem['codigo']+"</td><td>"+resultadoItem['descripcion']+"</td><td>$ "+miFormato(precioImprimible)+"</td><td>"+r['ncantidad']+"</td><td>$ "+miFormato(importeImprimible)+"</td><td class='text-center'><button class='delFila btn btn-danger btn-xs'><i class='fa fa-times-circle' aria-hidden='true'></i></button></td><td class='hidden'>"+importe+"</td></tr>");
                  }
...
$("body").on('click', '.cobrar', function () {
  ...
  /*if(tipo==1){
  cobrart(total,idVenta,impuesto,descuento);
  return false;
}*/
