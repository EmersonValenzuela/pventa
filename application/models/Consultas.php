...
class Consultas extends CI_Model
{
	...
	{
		$sql = "SELECT  vts.id,vts.idUsuario,vts.Total,vts.Fecha,vts.tipoPago
		,c.folio_factura,
		(select nombre from tb_proveedores where id = c.id_provedor) as proveedor
		FROM tb_ventas vts
		inner join tb_compras c on vts.id = c.idVenta
		WHERE vts.Total>0";
		$query = $this->db->query($sql);
		return $query->result_array();
	}
	...
	{
		$sql = "SELECT mv.*, inv.descripcion, inv.precio
	...
	function getComprasPeriodo($fecha1="",$fecha2="")
	{
		$sql = "SELECT  vts.id,vts.idUsuario,vts.Total,vts.Fecha,vts.tipoPago
		,c.folio_factura,
		(select nombre from tb_proveedores where id = c.id_provedor) as proveedor
	...
	// {
	// 	$sql = "SELECT MAX(id)+1 as idcompra FROM tb_compras";
	// 	$row = $this->db->query($sql)->row();
	// 	return empty($row->idcompra)?1:$row->idcompra;
	// }

	public function getCompras()
	{
		$sql = "SELECT c.*,p.nombre as proveedor FROM tb_compras c
		left outer join tb_proveedores p on p.id = c.id_provedor";
		$query = $this->db->query($sql, array($idVenta) );
		return $query->result_array();
		///////////////
		$this->db->join('tb_proveedores', 'tb_proveedores.id = tb_compras.id_provedor');
		return $this->db->get('tb_compras');
	}
	...
	public function getCreditos()
	{
		...
		pt.nombre,
		vts.Total,
    sum(ab.monto) as abonos
		FROM tb_ventas vts
		inner join tb_creditos cr on vts.id = cr.idVenta
		inner join tb_clientes cl on vts.idCliente = cl.id
		inner join tb_plazo_tipo pt on pt.idPlazoTipo = cr.plazoTipo
		inner join (select * from tb_abonos) ab on vts.id = ab.idVenta
